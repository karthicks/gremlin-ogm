...
[.lead]
The `gremlin-objects` link:pom.xml[module] defines a library that puts an object-oriented spin on the gremlin property graph.
...
----
<dependency>
  <groupId>org.github.karthicks</groupId>
...
Let's consider the example of the `person` vertex, taken from the "modern" and "the crew" graphs defined in the link:https://github.com/apache/tinkerpop/tree/master/gremlin-core/tinkergraph-gremlin/src/main/java/org/apache/tinkerpop/gremlin/tinkergraph/structure/TinkerFactory.java[TinkerFactory].
...
Or, the good old fashioned way, using the link:./src/main/java/org/apache/tinkerpop/gremlin/object/provider/GraphFactory.java[GraphFactory]:
...
For users that don't use dependency injection, they may manually pass the `GraphTraversalSource` to the link:./src/main/java/org/apache/tinkerpop/gremlin/object/provider/GraphFactory.java[GraphFactory].
